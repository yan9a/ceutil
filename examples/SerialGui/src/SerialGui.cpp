...
bool MyApp::OnInit()
{
    ...
    _fra = new MyFrame(this, wxT("Serial Com"));
...
MyFrame::MyFrame(MyApp* app, const wxString& title)
       : wxFrame(NULL, wxID_ANY, title,wxDefaultPosition,wxSize(390, 280), wxDEFAULT_FRAME_STYLE ^ wxRESIZE_BORDER),m_timer(this, ID_TIMER), _app(app)
{
    // set the frame icon
...
void MyFrame::SetParity(wxCommandEvent& WXUNUSED(event))
{
	if (_app->_com->IsOpened()) {
		...
		wxString cdev = wxString::Format(wxT("%c"), _app->_com->GetParity());
...
void MyFrame::SetDataSize(wxCommandEvent& WXUNUSED(event))
{
	if (_app->_com->IsOpened()) {
		...
		long n = wxGetNumberFromUser(wxT("Enter the data size"), wxT("Data Size"), wxT("Set Data Size"), _app->_com->GetDataSize(), 5, 8);
		if (n >= 0) {
			_app->_com->SetDataSize(n);
		}
		txtRx->AppendText(wxString::Format(wxT("Data size: %ld\n"), _app->_com->GetDataSize()));
	}
}
...
void MyFrame::SetStopBits(wxCommandEvent& WXUNUSED(event))
{
	if (_app->_com->IsOpened()) {
		...
		long n = wxGetNumberFromUser(wxT("Enter the number of stop bits"), wxT("Data Size"), wxT("Set stop bits"), long(_app->_com->GetStopBits()), 1, 2);
		if (n > 0) {
			_app->_com->SetStopBits(float(n));
		}
		txtRx->AppendText(wxString::Format(wxT("Stop bits: %ld\n"), long(_app->_com->GetStopBits())));
	}
}
...
{
	if (!_app->_com->SetRTS(chkRTS->IsChecked())) {
...
{
	if (!_app->_com->SetDTR(chkDTR->IsChecked())) {
...
void MyFrame::UpdateCommStatus()
{
	bool s;
	bool v;
	v = _app->_com->GetCTS(s);
	if (s) chkCTS->SetValue(v);
	v = _app->_com->GetDSR(s);
