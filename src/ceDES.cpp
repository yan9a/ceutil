...
namespace ce {
	...
	const unsigned char ceDES::IP_table[64] = {
	...
	const unsigned char ceDES::PC1_table[56] = {
					   57, 49, 41, 33, 25, 17,  9,
						1, 58, 50, 42, 34, 26, 18,
					   10,  2, 59, 51, 43, 35, 27,
					   19, 11,  3, 60, 52, 44, 36,
	...
	//-----------------------------------------------------------------------------
	// Key shift table
	const unsigned char ceDES::LeftShift_table[16] = {
						1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1 };
	//-----------------------------------------------------------------------------
	// Key Compression permutation table
	// 56 bit key data -> 48 bit 
	const unsigned char ceDES::PC2_table[48] = {
	...
	const unsigned char ceDES::E_table[48] = {
					   32,  1,  2,  3,  4,  5,
						4,  5,  6,  7,  8,  9,
						8,  9, 10, 11, 12, 13,
					   12, 13, 14, 15, 16, 17,
					   ...
					   28, 29, 30, 31, 32,  1 };
	//-----------------------------------------------------------------------------
	const unsigned char ceDES::S[8][4][16] = {
			{
	...
	const unsigned char ceDES::P_table[32] = {
	...
	// 64 bit data -> 64 bit data
	const unsigned char ceDES::InverseIP_table[64] = {
	...
	//-----------------------------------------------------------------------------
	unsigned char ceDES::Kn[7];
	unsigned char ceDES::Rn_1[6];
//-----------------------------------------------------------------------------
...
void ceDES::Encrypt(unsigned char *dMes,unsigned char *dKey)
{
	...
	Permutate(dMes,8,(unsigned char *)IP_table,8);//64 bit output
	Permutate(dKeyC,8,(unsigned char *)PC1_table,7);//56 bit output	
	for(n=0;n<16;n++)
	{
		LeftShift(dKeyC,LeftShift_table[n]);
		for(i=0;i<7;i++) Kn[i]=dKeyC[i];
		Permutate(Kn,7,(unsigned char *)PC2_table,6);//48 bit output			
	...
	Permutate(dMes,8,(unsigned char *)InverseIP_table,8);//64 bit output
}
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
...
void ceDES::Decrypt(unsigned char *dMes,unsigned char *dKey)
{
	...
	{		
		...
		for(i=0;i<4;i++) dMes[i]=dMes[i+4];		
		for(i=0;i<4;i++) dMes[i+4]=Rn_1[i];			
		RightShift(dKeyC,LeftShift_table[15-n]);
	}	
	Permutate(dMes,8,(unsigned char *)InverseIP_table,8);//64 bit output
}
//-----------------------------------------------------------------------------
...
void ceDES::F(unsigned char *R,unsigned char *K)
{
	...
	Permutate(R,4,(unsigned char *)P_table,4);//32 bit output
}
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
//dMes=Message to encrypt
...
//-----------------------------------------------------------------------------
// DES encryption, DES if dKey is 8 bytes, and TDES if it is 16 bytes
std::string ceDES::Encrypt(std::string dMes, std::string dKey)
{
	unsigned char buf[8];
	dMes.resize(8,'\0');
	memcpy(buf, dMes.c_str(), 8);

	dKey.resize(std::max<size_t>(dKey.length(), 8), '\0');

	// check if TDES to use
	if (dKey.length() > 8) { 
		dKey.resize(std::max<size_t>(dKey.length(), 16), '\0');
		Encrypt(buf, (unsigned char*)(dKey.c_str()), (unsigned char*)(dKey.c_str() + 8));
	}
	// for DES
	else {
		Encrypt(buf, (unsigned char*)(dKey.c_str()));
	}
	std::string r((char*)buf, 8);
	return r;
}
//-----------------------------------------------------------------------------
// DES decryption, DES if dKey is 8 bytes, and TDES if it is 16 bytes
std::string ceDES::Decrypt(std::string dMes, std::string dKey)
{
	unsigned char buf[8];
	dMes.resize(8, '\0');
	memcpy(buf, dMes.c_str(), 8);
	...
	if (dKey.length() > 8) {
		dKey.resize(std::max<size_t>(dKey.length(), 16), '\0');
		Decrypt(buf, (unsigned char*)(dKey.c_str()), (unsigned char*)(dKey.c_str() + 8));
	}
	// for DES
	else {
		Decrypt(buf, (unsigned char*)(dKey.c_str()));
	}
	std::string r((char*)buf, 8);
	return r;
}
